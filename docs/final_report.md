...
**Plain language interpretation of the preferred specification.** A 1% higher lagged bond-lodgement count in a Melbourne suburb is associated with about 0.019 percentage points lower rent growth in the subsequent quarter, holding constant every time-invariant suburb characteristic, every common Melbourne-wide time shock, and the lagged rent level. Equivalently, a 100% increase in turnover (a doubling) corresponds to about 1.3 percentage points lower rent growth - a substantial number relative to the sample mean of 1.3% per quarter. The within-suburb partial R² is modest (~0.5%, conditional on the fixed effects), which is expected for quarterly rent dynamics where most variation is idiosyncratic.

**Why the rent control matters so much.** Column 4 differs from column 3 only by the addition of log(lag rent), yet the turnover coefficient swings by more than two points. This is a textbook case of omitted variable bias (Lecture 6 in this course). The bias formula is β̃ = β̂ + δ̂ · ρ, where β̃ is the short-regression slope, β̂ is the long-regression slope, δ̂ is the coefficient on the omitted variable, and ρ is the within-suburb correlation between turnover and the rent level. In our panel, log turnover and log rent are positively correlated within suburbs (denser areas have both more turnover and higher rents); log lag rent has a strongly negative coefficient on growth (mean reversion); the product is a positive bias of about +2 percentage points. That is why the column-3 estimate looks like zero while the column-4 estimate is sharply negative. Reporting both columns is the honest way to show this.
...
![Figure 1: Turnover coefficient across the four main specifications, 95% CIs. The estimate is essentially zero in the pooled OLS, drifts negative with suburb FE, returns to near-zero with quarter FE, and becomes precisely negative only when log(lag rent) is added as a control. Source: code/03_analysis.ipynb §3.](../outputs/coefficient_plot.png)
...
- **Inference (C2, C3) - survives.** Switching from cluster-by-suburb to HC3 only or to cluster-by-region leaves the point estimate identical (SE choices do not move β̂) and the t-statistic above 3 in all three columns. The HC3 SE (0.351) is about 20% smaller than the cluster-by-suburb SE (0.439), the expected direction for a panel with within-suburb correlation. Cluster-by-region (9 clusters) is below the MHE rule-of-thumb threshold of 42 clusters and is reported as a sensitivity rather than a definitive number.
...
![Figure 2: Turnover coefficient across seven robustness specifications, 95% CIs. The main estimate (C1) is shown in blue; the OVB-revealing failure (C4) in orange; the pre-2018 placebo (C7) in green. The headline survives 6 of the 7 variations; the 1 failure is structural and explainable as omitted variable bias. Source: code/04_robustness.ipynb §6.](../outputs/robustness_coefficient_plot.png)
